...
// Funci√≥n para dibujar un car√°cter
void ILI9486_DrawChar(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bg_color, uint8_t scale) {
   ...
   // Validar coordenadas
    ...
    // Mapear caracter al Ìndice de la tabla 
    // Convertir caracter a Ìndice de tabla (32-126)
    if(c < 32 || c > 126) {
        char_index = 0; // Caracter no v·lido, usar espacio
    } else {
        char_index = c - 32;
    }
    ...
    for(uint8_t col = 0; col < 5; col++) {
        ...
        for(uint8_t row = 0; row < 7; row++) {
            ...
            // Verificar si el bit est· activo en los datos de la fuente
